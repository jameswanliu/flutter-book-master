...
class RankBookList extends StatefulWidget {
  ...
  RankBookList(this.title, this.id);
...
class RankBookListState extends State<RankBookList>
  ...
  final List<String> tiltes = ['周榜', '月榜', '总榜'];
  ...
  void initState() {
    super.initState();
    _controller = new TabController(length: tiltes.length, vsync: this);
  }
...
class BookListViewState extends State<BookListView>
  ...
  _getData() {
    BookApiRepository.getInstance().getRankBookList(widget.id).then((data) {
      setState(() {
        if (data != null) list.addAll(_rankBean.books);
        isLoading = false;
      });
    });
  }
